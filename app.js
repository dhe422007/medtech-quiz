...
async function boot(){
  ...
  try{
    ...
    state.all=raw.map(normalize).filter(q=>q.choices.length===5); // 必ず5択
  }catch(e){
    console.error(e);
...
function normalize(src){
  ...
  // choices: choices[] が無ければ choice1..5 から必ず5つ構成（空欄も維持）
  let choices = Array.isArray(q.choices) ? q.choices : [q.choice1,q.choice2,q.choice3,q.choice4,q.choice5];
  choices = (choices||[]).slice(0,5).map(v => v==null ? '' : String(v));
  while(choices.length<5) choices.push(''); // 5つ保証
  q.choices = choices;

  // answerIndex（0始まりの想定／複数可）
  let ans = q.answerIndex;
  if(ans===undefined||ans===null){
    const s=String(q.answer ?? q.解答 ?? '').trim();
    if(s){
      const nums = s.split(/[,/・\s]+/).filter(Boolean).map(x=>parseInt(x,10));
      ans = nums.length>1 ? Array.from(new Set(nums)).sort((a,b)=>a-b) : (nums[0] ?? 0);
    }else{
      ans = 0;
    }
  }
  ...
  // image: 拡張子なしでもOK用に imageBase を持たせる
  let img = (q.image ?? '').toString().trim();
  if(img && !/\.(png|jpg|jpeg|gif|webp|svg)$/i.test(img)) img = img.replace(/\/+$/,'');
  q.imageBase = img;
...
function applyFilters(){
  ...
  state.filtered=state.all.filter(q=>{const t=q.tags||[];return (year===''||t.includes(year)) && (tag===''||t.includes(tag));});
...
function startFromHome({year='',tag=''}={}){
  ...
  if(ySel) ySel.value=year; if(tSel) tSel.value=tag;
  state.yearFilter=year; state.tagFilter=tag;
  applyFilters();
  const n=state.filtered.length;
  if(!n){ const a=q$('#alert'); if(a){a.textContent='該当問題が0件のため開始できません。'; a.classList.remove('hidden');} return; }
  state.session={startedAt:Date.now(),correct:0,total:n};
  showQuiz(); render();
}
...
function render(){
  ...
  q$('#qmeta').textContent=`ID：${q.id??`idx:${state.idx}`}`;
  renderImage(q);
  renderChoices(q);
...
function renderImage(q){
  ...
  let i=0; const img=document.createElement('img');
  img.style.maxWidth='100%'; img.style.borderRadius='12px'; img.style.border='1px solid rgba(15,23,42,.1)';
  const set=()=>{const src=/\.[a-z]{3,4}$/i.test(base)?base:(base+TRY_EXTS[i]); img.src=src; img.alt='問題図';};
  img.onerror=()=>{i++; if(i<TRY_EXTS.length) set(); else { node.classList.add('hidden'); node.innerHTML=''; } };
  set(); node.innerHTML=''; node.appendChild(img);
}
...
function renderChoices(q){
  ...
  order.forEach(idx=>{
    const val=String(q.choices[idx]??'');
    const btn=document.createElement('button');
    btn.className='choice'; btn.setAttribute('data-idx', String(idx)); btn.setAttribute('aria-pressed','false');

    if(/^assets\//.test(val)||/^images\//.test(val)){ // 画像選択肢
      const base=/\.[a-z]{3,4}$/i.test(val)?val:val.replace(/\/+$/,'');
      const img=document.createElement('img'); img.style.maxWidth='100%'; img.style.display='block';
      let k=0; const set=()=>{img.src=/\.[a-z]{3,4}$/i.test(base)?base:(base+TRY_EXTS[k]);};
      img.onerror=()=>{k++; if(k<TRY_EXTS.length) set(); else btn.textContent='(画像なし)';};
      set(); btn.appendChild(img);
    }else{
    ...
    btn.addEventListener('click',()=>{
      if(multi){
        ...
        q$$('#choices .choice').forEach(el=>{el.classList.remove('selected'); el.setAttribute('aria-pressed','false');});
...
function updateNextButtonAvailability(q){
  const selected=q$$('#choices .choice.selected'); const next=q$('#nextBtn');
  if(Array.isArray(q.answerIndex)){
    const need=q.answerIndex.length; next.disabled=(selected.length!==need); next.title=selected.length!==need?`この問題は ${need} 個選んでください`:'';
  }else{
    next.disabled=(selected.length!==1); next.title=selected.length!==1?'選択肢を1つ選んでください':'';
  }
}
...
function grade(){
  ...
  const result=isCorrect(selected, q.answerIndex);
  const answerSet=new Set(Array.isArray(q.answerIndex)?q.answerIndex:[q.answerIndex]);

  q$$('#choices .choice').forEach(el=>{
    const i=Number(el.getAttribute('data-idx'));
    if(answerSet.has(i)) el.classList.add('correct');
    if(selected.includes(i) && !answerSet.has(i)) el.classList.add('incorrect');
  });

  const ex=q$('#explain'); ex.classList.remove('hidden');
  const multi=Array.isArray(q.answerIndex);
  const head = result.ok ? (multi?`🎉 全て正解です（${result.total}/${result.total}）`:'🎉 正解です')
                         : (multi?`▲ 部分正解：${result.partial}/${result.total}。残りの選択肢も確認しましょう。`:`✕ 不正解。もう一度見直しましょう。`);
  ex.innerHTML = `<div>${head}</div>${q.explanation?`<div style="margin-top:6px;">${esc(q.explanation)}</div>`:''}`;

  if(state.idx>=state.filtered.length-1) q$('#nextBtn').textContent='結果を見る';
  else q$('#nextBtn').textContent='次へ';
...
function next(){
  ...
  if(btn.textContent.includes('結果')) { showResult(); return; }
...
function bindUI(){
  ...
  q$('#statsBtn')?.addEventListener('click', ()=>alert('成績・弱点は後日実装'));
...
function isCorrect(selected, answerIndex){
